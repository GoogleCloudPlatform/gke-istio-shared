...
kubectl apply -f <("${ISTIO_DIR}"/bin/istioctl kube-inject -f \
  "${ISTIO_DIR}"/samples/bookinfo/kube/bookinfo.yaml)
...
for DEST_RULE in "productpage" "reviews" "ratings" "details" "bookinfo"; do
...
"${ISTIO_DIR}"/bin/istioctl create -f \
  "${ISTIO_DIR}"/samples/bookinfo/routing/bookinfo-gateway.yaml

if [[ ${ISTIO_AUTH_POLICY} == "MUTUAL_TLS" ]]; then
  "${ISTIO_DIR}"/bin/istioctl create -f \
    "${ISTIO_DIR}"/samples/bookinfo/routing/route-rule-all-v1-mtls.yaml
else
  "${ISTIO_DIR}"/bin/istioctl create -f \
    "${ISTIO_DIR}"/samples/bookinfo/routing/route-rule-all-v1.yaml
fi

"${ISTIO_DIR}"/bin/istioctl replace -f \
  "${ISTIO_DIR}"/samples/bookinfo/routing/route-rule-reviews-v3.yaml
